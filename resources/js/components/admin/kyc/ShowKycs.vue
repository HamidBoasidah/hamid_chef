<template>
  <div class="overflow-hidden">
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
          ...
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('applicant')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('kyc.applicant') }}</p>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('document_type')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('kyc.documentType') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill="" />
                  </svg>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('is_verified')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('kyc.verificationStatus') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" fill="" />
                  </svg>
        ...
        <tbody>
          <tr
            v-for="kyc in paginatedData"
            :key="kyc.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': kyc.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="kyc.selected" @change="updateSelectAll" />
                  <span
                    :class="kyc.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="kyc.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                  <span class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ applicantName(kyc) }}</span>
                  <span class="text-xs text-gray-500 dark:text-gray-400">{{ kyc.user?.email }}</span>
                </div>
              </div>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center gap-2">
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button
                    :disabled="!canDelete"
                    @click="handleDeleteClick(kyc.id)"
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteKycConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['kycs.create', 'kycs.store']))
const canView = computed(() => hasAnyPermission(['kycs.view', 'kycs.show', 'kycs.read']))
const canEdit = computed(() => hasAnyPermission(['kycs.update', 'kycs.edit']))
const canDelete = computed(() => hasAnyPermission(['kycs.delete', 'kycs.destroy']))

const search = ref('')
const sortColumn = ref('applicant')
const sortDirection = ref('asc')
const perPage = ref(props.kycs?.per_page ?? 10)
const currentPage = ref(props.kycs?.current_page ?? 1)
...
const applicantName = (kyc) => {
  if (kyc.full_name) return kyc.full_name
  const first = kyc.user?.first_name ?? ''
  const last = kyc.user?.last_name ?? ''
  const combined = `${first} ${last}`.trim()
  return combined || kyc.user?.name || t('kyc.unknownApplicant')
}

const filteredData = computed(() => {
  const searchLower = search.value.toLowerCase().trim()
  return (props.kycs?.data ?? [])
    .filter((kyc) => {
...
const sortableValue = (kyc, column) => {
  switch (column) {
    case 'document_type':
      return documentTypeLabel(kyc.document_type).toLowerCase()
    case 'status':
...
const totalEntries = computed(() => props.kycs?.total ?? filteredData.value.length)
const startEntry = computed(() => props.kycs?.from ?? (filteredData.value.length ? 1 : 0))
const endEntry = computed(() => props.kycs?.to ?? filteredData.value.length)
const totalPages = computed(() => props.kycs?.last_page ?? 1)
...
const toggleSelectAll = () => {
  filteredData.value.forEach((kyc) => {
    // eslint-disable-next-line no-param-reassign
    kyc.selected = selectAll.value
  })
}

const updateSelectAll = () => {
  const items = filteredData.value
  selectAll.value = items.length > 0 && items.every((kyc) => kyc.selected)
}

const goToCreate = () => router.visit(route('admin.kycs.create'))
const goToView = (id) => router.visit(route('admin.kycs.show', id))
const goToEdit = (id) => router.visit(route('admin.kycs.edit', id))
...
const closeDeleteModal = () => {
  ...
  kycToDeleteId.value = null
}

const confirmDelete = () => {
  if (!kycToDeleteId.value) return
  router.delete(route('admin.kycs.destroy', kycToDeleteId.value), {
    preserveScroll: true,
    onSuccess: () => {
      success(t('kyc.kycDeletedSuccessfully'))
      closeDeleteModal()
    },
    onError: () => {
      error(t('kyc.kycDeletionFailed'))
      closeDeleteModal()
    },
  })
}
