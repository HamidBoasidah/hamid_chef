...
const props = defineProps({ admin: Object, roles: Array })
...
watch(() => props.admin, (admin) => {
  if (admin) {
    ...
    form.phone_number = admin.phone_number || ''
    form.role_id = admin.role?.id || ''
  }
})

function submit() {
  form.patch(route('admin.admins.update', props.admin.id), {
    onSuccess: () => {
      // handled by server or redirect
    },
  })
}
</script>
