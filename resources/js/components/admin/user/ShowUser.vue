<template>
  <div class="space-y-6">
    ...
    >
      ...
      <div class="p-4 sm:p-6">
        <div class="grid grid-cols-1 gap-x-5 gap-y-6 md:grid-cols-2">
          <!-- User Name -->
          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-500 dark:text-gray-400">{{ t('adduser.userName') }}</label>
            <p class="text-base text-gray-800 dark:text-white/90">{{ user.name }}</p>
          </div>
          ...
          <div>
            ...
            <p class="text-base capitalize text-gray-800 dark:text-white/90">{{ displayName(user.role) }}</p>
          </div>
            <!-- User Status -->
            <div>
          ...
          <div class="md:col-span-2">
            ...
            <p class="text-base text-gray-800 dark:text-white/90">{{ user.address || 'N/A' }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div class="p-4 sm:p-6">
        <div v-if="user.attachment" class="relative flex justify-center p-4">
          <img :src="`/storage/${user.attachment}`" alt="User Image" class="max-h-64 rounded-lg border border-gray-200 object-contain dark:border-gray-800" />
        </div>
