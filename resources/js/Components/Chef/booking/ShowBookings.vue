<template>
  <div class="overflow-hidden">
    ...
    <div v-if="paginatedData.length" class="grid grid-cols-1 gap-4 px-4 py-4 border border-t-0 border-gray-200 rounded-b-xl dark:border-gray-800 sm:grid-cols-2 xl:grid-cols-3">
      <article v-for="booking in paginatedData" :key="booking.id" class="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/5">
        <div class="relative p-5 pb-9">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="max-w-xs text-sm text-gray-500 dark:text-gray-400">
                {{ t('booking.chef') }}: {{ booking.chef?.name }}
              </p>
            </div>
          </div>
        </div>
    ...
    <DangerAlert :isOpen="isDeleteModalOpen" :title="t('messages.areYouSure')" :message="t('messages.deleteBookingConfirmation')" @close="closeDeleteModal" @confirm="confirmDelete" />
...
const canCreate = computed(() => hasAnyPermission(['bookings.create','bookings.store','bookings.add']))
const canView = computed(() => hasAnyPermission(['bookings.view','bookings.show','bookings.read']))
...
function openDeleteModal(id) { bookingToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id) { if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal() { isDeleteModalOpen.value = false; bookingToDeleteId.value = null }
function confirmDelete() {
  if (bookingToDeleteId.value) {
    router.delete(route('admin.bookings.destroy', bookingToDeleteId.value), {
      onSuccess: () => { success(t('booking.booking_deleted_successfully')); closeDeleteModal() },
      onError: () => { error(t('booking.booking_deletion_failed')); closeDeleteModal() },
      preserveScroll: true,
    })
  }
}
...
const formatTime = (timeString) => {
  ...
  // ISO datetime (e.g. 2025-12-21T08:00:00.000000Z)
  if (timeString.includes('T')) {
    const d = new Date(timeString)
    if (!isNaN(d)) return d.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })
  }
  // If already HH:mm:ss or HH:mm, normalize to HH:mm
...
const filteredData = computed(() => {
  ...
  return (props.bookings?.data || [])
    .filter((booking) => 
      ...
      booking.chef?.name?.toLowerCase().includes(searchLower) ||
...
const endEntry = computed(() => props.bookings?.to || filteredData.value.length)
...
watch(() => props.bookings?.current_page, (val) => { currentPage.value = typeof val === 'number' ? val : 1 })
watch(() => props.bookings?.per_page, (val) => { if (typeof val === 'number') perPage.value = val })
