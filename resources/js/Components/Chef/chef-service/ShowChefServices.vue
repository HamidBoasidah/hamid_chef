<template>
  <div class="overflow-hidden">
    <div class="flex flex-col gap-2 px-4 py-4 border border-b-0 border-gray-200 rounded-b-none rounded-xl dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <button @click="handleCreateClick" class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-[11px] text-sm font-medium text-white transition sm:w-auto">
          ...
          {{ t('chef_services.addChefService') }}
        </button>
      </div>
    </div>
    ...
    <div v-if="paginatedData.length" class="grid grid-cols-1 gap-4 px-4 py-4 border border-t-0 border-gray-200 rounded-b-xl dark:border-gray-800 sm:grid-cols-2 xl:grid-cols-3">
      <article v-for="service in paginatedData" :key="service.id" class="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/5">
        <div class="relative p-5 pb-9">
          <div class="flex items-start gap-3">
            <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 dark:bg-brand-500/15 dark:text-brand-200">
              <img
                v-if="service.feature_image"
                :src="`/storage/${service.feature_image}`"
                class="h-12 w-12 rounded-2xl object-cover"
                alt=""
              />
              <TaskIcon v-else class="h-6 w-6" />
            </div>
            <div>
              ...
              <p class="max-w-xs text-sm text-gray-500 dark:text-gray-400">
                {{ service.chef_name || t('chef_services.noChefAssigned') }}
              </p>
              <div class="mt-1 flex items-center gap-2">
                ...
                >
                  {{ service.service_type === 'hourly' ? t('chef_services.hourlyService') : t('chef_services.packageService') }}
                </span>
              </div>
              <!-- Tags -->
              <div v-if="service.tags && service.tags.length > 0" class="mt-2 flex flex-wrap gap-1">
                <span
        ...
        <div class="flex items-center justify-between border-t border-gray-200 px-5 py-4 dark:border-gray-800">
          <div class="flex-1">
            <p class="text-xs font-medium uppercase tracking-wide text-gray-400">
              {{ service.service_type === 'hourly' ? t('chef_services.hourlyRate') : t('chef_services.packagePrice') }}
            </p>
            <div class="mt-1 flex items-baseline gap-1">
              <p class="text-2xl font-bold text-gray-800 dark:text-white/90">
                {{ service.service_type === 'hourly' ? service.hourly_rate : service.package_price }}
              </p>
              <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                {{ service.service_type === 'hourly' ? t('chef_services.perHour') : t('common.sar') }}
              </p>
            </div>
            <div class="mt-1 space-y-1">
              <p v-if="service.service_type === 'package' && service.min_hours" class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('chef_services.minHours') }}: {{ service.min_hours }} {{ t('chef_services.hours') }}
              </p>
              <p v-if="service.max_guests_included" class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('chef_services.maxGuestsIncluded') }}: {{ service.max_guests_included }}
              </p>
            </div>
          </div>
          <span
    ...
    <div v-else class="border border-t-0 rounded-b-xl border-gray-200 px-4 py-12 text-center text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
      {{ t('chef_services.noServicesFound') }}
    </div>
    ...
    <DangerAlert :isOpen="isDeleteModalOpen" :title="t('messages.areYouSure')" :message="t('messages.deleteChefServiceConfirmation')" @close="closeDeleteModal" @confirm="confirmDelete" />
...
const canCreate = computed(() => hasAnyPermission(['chef-services.create','chef-services.store','chef-services.add']))
const canView = computed(() => hasAnyPermission(['chef-services.view','chef-services.show','chef-services.read']))
const canEdit = computed(() => hasAnyPermission(['chef-services.update','chef-services.edit']))
const canDelete = computed(() => hasAnyPermission(['chef-services.delete','chef-services.destroy']))
...
function handleCreateClick() {
  ...
  router.visit(route('admin.chef-services.create'))
}

function handleViewClick(id) {
  if (!canView.value) return
  router.visit(route('admin.chef-services.show', id))
}

function handleEditClick(id) {
  if (!canEdit.value) return
  router.visit(route('admin.chef-services.edit', id))
}
...
function confirmDelete() {
  if (serviceToDeleteId.value) {
    router.delete(route('admin.chef-services.destroy', serviceToDeleteId.value), {
...
function toggleServiceStatus(service) {
  ...
  const url = wasActive ? route('admin.chef-services.deactivate', { id: service.id }) : route('admin.chef-services.activate', { id: service.id })
  ...
  return (props.services?.data || [])
    .filter((service) => {
      return service.name?.toLowerCase().includes(searchLower) || 
             ...
             (service.tags || []).some(tag => tag.name?.toLowerCase().includes(searchLower))
    })
    .sort((a,b) => {
...
watch(() => props.services?.current_page, (val) => { currentPage.value = typeof val === 'number' ? val : 1 })
watch(() => props.services?.per_page, (val) => { if (typeof val === 'number') perPage.value = val })
