<template>
              ...
              <label :for="'toggle-' + category.id" class="cursor-pointer">
                <div class="relative">
                  ...
                  <div
                    ...
                    :class="category.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
                  ></div>
                  <div
                    :class="[category.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']"
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button
                    :disabled="!canView"
                    @click="handleViewClick(category.id)"
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteCategoryConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['categories.create', 'categories.store', 'categories.add']))
const canView = computed(() => hasAnyPermission(['categories.view', 'categories.show', 'categories.read']))
const canEdit = computed(() => hasAnyPermission(['categories.update', 'categories.edit']))
const canDelete = computed(() => hasAnyPermission(['categories.delete', 'categories.destroy']))
...
function confirmDelete() {
  if (categoryToDeleteId.value) {
    router.delete(route('admin.categories.destroy', categoryToDeleteId.value), {
      onSuccess: () => {
        success(t('categories.categoryDeletedSuccessfully'))
        closeDeleteModal()
      },
      onError: () => {
        error(t('categories.categoryDeleteFailed'))
        closeDeleteModal()
      },
      preserveScroll: true,
    })
...
function toggleCategoryStatus(category) {
  const wasActive = category.is_active
  category.is_active = !wasActive
  const url = wasActive ? route('admin.categories.deactivate', { id: category.id }) : route('admin.categories.activate', { id: category.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => {
...
const totalEntries = computed(() => props.categories?.total || filteredData.value.length)
const startEntry = computed(() => props.categories?.from || 1)
const endEntry = computed(() => props.categories?.to || filteredData.value.length)
const totalPages = computed(() => props.categories?.last_page || 1)
...
const updateSelectAll = () => {
  ...
  selectAll.value = items.length > 0 && items.every((category) => category.selected)
}
