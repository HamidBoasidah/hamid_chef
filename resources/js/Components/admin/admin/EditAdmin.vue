	...
	<div class="space-y-6">
				...
				<form @submit.prevent>
					<div class="grid grid-cols-1 gap-5 md:grid-cols-2">
						...
						<div>
							...
							/>
							<p v-if="form.errors.first_name" class="mt-2 text-sm text-error-500">{{ form.errors.first_name }}</p>
						</div>
						...
						<div>
							...
							<input
								...
								class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
							/>
							<p v-if="form.errors.last_name" class="mt-2 text-sm text-error-500">{{ form.errors.last_name }}</p>
						</div>
						...
						<div>
							...
							<p v-if="form.errors.email" class="mt-2 text-sm text-error-500">{{ form.errors.email }}</p>
						</div>
              ...
              <div class="relative">
                ...
                />
						...
						<div>
							...
							<div class="relative">
								...
								>
									<option value="" disabled>
										{{ t('admins.selectUserType') }}
									</option>
									<option
										v-for="role in roles"
										:key="role.id"
										:value="role.id"
										class="text-gray-800 dark:bg-gray-900 dark:text-gray-300"
									>
										{{ displayName(role) }}
									</option>
								</select>
							...
							<p v-if="form.errors.role_id" class="mt-2 text-sm text-error-500">{{ form.errors.role_id }}</p>
						</div>
						...
						<div class="md:col-span-2">
							...
							<input
								...
								:placeholder="t('admins.addressPlaceholder')"
								class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
							/>
							<p v-if="form.errors.address" class="mt-2 text-sm text-error-500">{{ form.errors.address }}</p>
						</div>
						...
						<div class="md:col-span-2 grid grid-cols-1 gap-5 md:grid-cols-2">
							<div>
								...
								<input
									...
									class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
								/>
								<p v-if="form.errors.facebook" class="mt-2 text-sm text-error-500">{{ form.errors.facebook }}</p>
							</div>
							<div>
								<label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
									{{ t('profile.labels.x') }}
								...
								<input
									...
									class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
								/>
								<p v-if="form.errors.x_url" class="mt-2 text-sm text-error-500">{{ form.errors.x_url }}</p>
							</div>
							<div>
								<label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
								...
								<input
									...
									class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
								/>
								<p v-if="form.errors.linkedin" class="mt-2 text-sm text-error-500">{{ form.errors.linkedin }}</p>
							</div>
							<div>
								<label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
								...
								<input
									...
									class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
								/>
								<p v-if="form.errors.instagram" class="mt-2 text-sm text-error-500">{{ form.errors.instagram }}</p>
							</div>
						</div>
						...
						<div>
							...
							<p v-if="form.errors.password" class="mt-2 text-sm text-error-500">{{ form.errors.password }}</p>
						</div>
						...
						<div>
							...
							<p v-if="form.errors.password_confirmation" class="mt-2 text-sm text-error-500">{{ form.errors.password_confirmation }}</p>
						</div>
					</div>
				</form>
			</div>
		...
		<div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
			<Link
				...
				class="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 ring-1 ring-gray-300 shadow-theme-xs transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]"
			>
...
const form = useForm({
	...
	is_active: props.admin?.is_active ?? false,
	password: '',
	password_confirmation: '',
	avatar: null,
	facebook: props.admin?.facebook ?? '',
	x_url: props.admin?.x_url ?? '',
	linkedin: props.admin?.linkedin ?? '',
	instagram: props.admin?.instagram ?? '',
})

const showPassword = ref(false)
// fileInput removed — ImageUploadBox handles file input internally

const togglePasswordVisibility = () => {
	showPassword.value = !showPassword.value
}

// ImageUploadBox handles file selection and preview. form.avatar is seeded with existing URL when present.

const update = () => {
	// إذا لم يتم رفع صورة جديدة، لا نرسل حقل avatar
	const formData = { ...form.data() }
	if (!form.avatar) {
		delete formData.avatar
	}
	
	form.transform(() => formData).post(route('admin.admins.update', props.admin.id), {
		preserveScroll: true,
		onSuccess: () => {
			success(t('admins.userUpdatedSuccessfully'))
		},
		onError: () => {
			error(t('admins.userUpdateFailed'))
		},
	})
}
