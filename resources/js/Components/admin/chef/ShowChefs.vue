<template>
  <div class="overflow-hidden">
    <div class="flex flex-col gap-2 px-4 py-4 border border-b-0 border-gray-200 rounded-b-none rounded-xl dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
      <div class="flex items-center gap-3">
        ...
        <div class="relative z-20 bg-transparent">
          <select v-model="perPage" class="w-full py-2 pl-3 pr-8 text-sm text-gray-800 bg-transparent border border-gray-300 rounded-lg appearance-none dark:bg-dark-900 h-9 bg-none shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" :class="{ 'text-gray-500 dark:text-gray-400': perPage !== '' }">
        ...
        <div class="relative">
          <button class="absolute text-gray-500 -translate-y-1/2 left-4 top-1/2 dark:text-gray-400">
            <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M3.04199 9.37363C3.04199 5.87693 5.87735 3.04199 9.37533 3.04199C12.8733 3.04199 15.7087 5.87693 15.7087 9.37363C15.7087 12.8703 12.8733 15.7053 9.37533 15.7053C5.87735 15.7053 3.04199 12.8703 3.04199 9.37363ZM9.37533 1.54199C5.04926 1.54199 1.54199 5.04817 1.54199 9.37363C1.54199 13.6991 5.04926 17.2053 9.37533 17.2053C11.2676 17.2053 13.0032 16.5344 14.3572 15.4176L17.1773 18.238C17.4702 18.5309 17.945 18.5309 18.2379 18.238C18.5308 17.9451 18.5309 17.4703 18.238 17.1773L15.4182 14.3573C16.5367 13.0033 17.2087 11.2669 17.2087 9.37363C17.2087 5.04817 13.7014 1.54199 9.37533 1.54199Z" fill="" />
            </svg>
          </button>
          <input v-model="search" type="text" :placeholder="t('datatable.searchPlaceholder')" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-11 pr-4 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 xl:w-[300px]" />
        </div>

        <button @click="handleCreateClick" class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-[11px] text-sm font-medium text-white transition sm:w-auto">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M5 10.0002H15.0006M10.0002 5V15.0006" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
          {{ t('chefs.addChef') }}
        </button>
      </div>
    </div>

    <div v-if="paginatedData.length" class="grid grid-cols-1 gap-4 px-4 py-4 border border-t-0 border-gray-200 rounded-b-xl dark:border-gray-800 sm:grid-cols-2 xl:grid-cols-3">
      <article v-for="chef in paginatedData" :key="chef.id" class="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/5">
        <div class="relative p-5 pb-9">
          <div class="flex items-start gap-3">
            <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 dark:bg-brand-500/15 dark:text-brand-200">
              <img
                v-if="chef.logo"
                :src="`/storage/${chef.logo}`"
                class="h-12 w-12 rounded-2xl object-cover"
                alt=""
              />
              <ChefIcon v-else class="h-6 w-6" />
            </div>
            <div>
        ...
        <div class="flex items-center justify-between border-t border-gray-200 px-5 py-4 dark:border-gray-800">
          ...
          <span
            ...
            "
          >
            {{ chef.is_active ? t('common.active') : t('common.inactive') }}
          </span>
        </div>
        ...
        <div class="flex items-center justify-between border-t border-gray-200 px-5 py-4 dark:border-gray-800">
          <div class="flex items-center gap-2">
            ...
            <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
              <button
                :disabled="!canDelete"
                @click="handleDeleteClick(chef.id)"
          ...
          <label :for="'toggle-' + chef.id" class="cursor-pointer">
            <div class="relative">
              <input
                type="checkbox"
                :id="'toggle-' + chef.id"
                ...
                @change="toggleChefStatus(chef)"
              />
              <div
                class="block h-6 w-11 rounded-full"
                :class="chef.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
              ></div>
              <div
                :class="chef.is_active ? 'translate-x-full rtl:-translate-x-full' : 'translate-x-0'"
    ...
    <div v-else class="border border-t-0 rounded-b-xl border-gray-200 px-4 py-12 text-center text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
      {{ t('chefs.noChefsFound') }}
    </div>

    <!-- Pagination Controls -->
    <div class="border border-t-0 rounded-b-xl border-gray-100 py-4 pl-[18px] pr-4 dark:border-gray-800">
      <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between">
        <p class="pb-3 text-sm font-medium text-center text-gray-500 border-b border-gray-100 dark:border-gray-800 dark:text-gray-400 xl:border-b-0 xl:pb-0 xl:text-left">{{ t('datatable.showing', { start: startEntry, end: endEntry, total: totalEntries }) }}</p>
    ...
    <DangerAlert :isOpen="isDeleteModalOpen" :title="t('messages.areYouSure')" :message="t('messages.deleteChefConfirmation')" @close="closeDeleteModal" @confirm="confirmDelete" />
...
import ChefIcon from '@/icons/ChefIcon.vue'

const { hasAnyPermission } = usePermissions()

const canCreate = computed(() => hasAnyPermission(['chefs.create','chefs.store','chefs.add']))
const canView = computed(() => hasAnyPermission(['chefs.view','chefs.show','chefs.read']))
const canEdit = computed(() => hasAnyPermission(['chefs.update','chefs.edit']))
const canDelete = computed(() => hasAnyPermission(['chefs.delete','chefs.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ chefs: Object })

const search = ref('')
const sortColumn = ref('name')
const sortDirection = ref('asc')
const currentPage = ref(props.chefs?.current_page ?? 1)
const perPage = ref(props.chefs?.per_page ?? 10)

function handleCreateClick() {
  if (!canCreate.value) return
  router.visit(route('admin.chefs.create'))
}

function handleViewClick(id) {
  if (!canView.value) return
  router.visit(route('admin.chefs.show', id))
}

function handleEditClick(id) {
  if (!canEdit.value) return
  router.visit(route('admin.chefs.edit', id))
}

// delete modal
const isDeleteModalOpen = ref(false)
const chefToDeleteId = ref(null)
function openDeleteModal(id) { chefToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id) { if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal() { isDeleteModalOpen.value = false; chefToDeleteId.value = null }
function confirmDelete() {
  if (chefToDeleteId.value) {
    router.delete(route('admin.chefs.destroy', chefToDeleteId.value), {
      onSuccess: () => { success(t('chefs.chefDeletedSuccessfully')); closeDeleteModal() },
      onError: () => { error(t('chefs.chefDeletionFailed')); closeDeleteModal() },
      preserveScroll: true,
    })
  }
}
...
function toggleChefStatus(chef) {
  const wasActive = chef.is_active
  chef.is_active = !wasActive
  const url = wasActive ? route('admin.chefs.deactivate', { id: chef.id }) : route('admin.chefs.activate', { id: chef.id })
  router.patch(url, {}, { preserveState: true, preserveScroll: true, onError: () => { chef.is_active = wasActive } })
}

const filteredData = computed(() => {
  const searchLower = search.value.toLowerCase()
  return (props.chefs?.data || [])
    .filter((chef) => chef.name?.toLowerCase().includes(searchLower) || (chef.address || '').toLowerCase().includes(searchLower))
    .sort((a,b) => {
...
const totalEntries = computed(() => props.chefs?.total || filteredData.value.length)
const startEntry = computed(() => props.chefs?.from || 1)
const endEntry = computed(() => props.chefs?.to || filteredData.value.length)
const totalPages = computed(() => props.chefs?.last_page || 1)
...
watch(() => props.chefs?.current_page, (val) => { currentPage.value = typeof val === 'number' ? val : 1 })
watch(() => props.chefs?.per_page, (val) => { if (typeof val === 'number') perPage.value = val })

const fetchPage = (page) => {
  const targetPage = page ?? currentPage.value
  router.get(window.location.pathname, { page: targetPage, per_page: perPage.value, search: search.value || undefined, sort: sortColumn.value, direction: sortDirection.value }, { preserveState: true, preserveScroll: true, replace: true })
}
