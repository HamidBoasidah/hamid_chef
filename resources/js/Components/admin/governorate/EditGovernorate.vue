<template>
  <div class="space-y-6">
    <!-- Governorate Description Section -->
    <div
      class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]"
    >
      <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
  <h2 class="text-lg font-medium text-gray-800 dark:text-white">{{ t('governorates.governorateInformation') }}</h2>
      </div>
      <div class="p-4 sm:p-6 dark:border-gray-800">
        <form @submit.prevent>
          <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            <!-- Governorate Name AR Input -->
            <div>
              <label
                for="governorate-name-ar"
                class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400"
                >{{ t('governorates.nameAr') }}</label
              >
              <input
                v-model="form.name_ar"
                type="text"
                id="governorate-name-ar"
                autocomplete="governorate-name-ar"
            ...
            <div>
              <label
                for="governorate-name-en"
                class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400"
                >{{ t('governorates.nameEn') }}</label
              >
              <input
                v-model="form.name_en"
                type="text"
                id="governorate-name-en"
                autocomplete="governorate-name-en"
                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
...
const form = useForm({
  ...
  name_ar: props.governorate.name_ar,
  name_en: props.governorate.name_en,
  is_active: props.governorate.is_active,

});


function update() {
  form.post(route('admin.governorates.update', props.governorate.id), {
    onSuccess: () => {
      success(t('governorates.governorateUpdatedSuccessfully'))
      // قد يعيد الخادم التوجيه؛ التوست سيظل ظاهراً بفضل مستضيف الإشعارات العالمي
    },
    onError: () => {
      error(t('governorates.governorateUpdateFailed'))
    },
    preserveScroll: true,
  });
}
